...
public final class MoveOrdering {
    ...
    private List<Move> orderImpl(final Board board) {
        ...
        for (final Move move : board.currentPlayer().getLegalMoves()) {
            ...
            if (moveTransition.getMoveStatus().isDone()) {
                ...
                final int currentValue = attackBonus + (board.currentPlayer().getAlliance().isWhite() ? min(moveTransition.getToBoard(), MoveOrdering.ORDER_SEARCH_DEPTH - 1) : max(moveTransition.getToBoard(), MoveOrdering.ORDER_SEARCH_DEPTH - 1));
                moveOrderEntries.add(new MoveOrderEntry(move, currentValue));
            }
        }
        moveOrderEntries.sort((o1, o2) -> Ints.compare(o2.getScore(), o1.getScore()));
    ...
    private static Collection<Move> calculateSimpleMoveOrder(final Collection<Move> moves) {
        final List<Move> sortedMoves = new ArrayList<>(moves);
        sortedMoves.sort((m1, m2) -> Booleans.compare(m2.isAttack(), m1.isAttack()));
        return sortedMoves;
    }
