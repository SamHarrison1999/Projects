...
public final class MiniMax implements MoveStrategy {
    ...
    public Move execute(final Board board) {
        ...
        for (final Move move : board.currentPlayer().getLegalMoves()) {
            ...
            if (moveTransition.getMoveStatus().isDone()) {
                ...
                currentValue = board.currentPlayer().getAlliance().isWhite() ? min(moveTransition.getToBoard(), this.searchDepth - 1) : max(moveTransition.getToBoard(), this.searchDepth - 1);
                System.out.println("\t" + this + " analyzing move (" +moveCounter + "/" +numMoves+ ") " + move + " scores " + currentValue + " " +this.freqTable[this.freqTableIndex]);
                this.freqTableIndex++;
    ...
    private static class FreqTableRow {
        ...
        @Override
        public String toString() {
            return BoardUtils.INSTANCE.getPositionAtCoordinate(this.move.getCurrentCoordinate()) + BoardUtils.INSTANCE.getPositionAtCoordinate(this.move.getDestinationCoordinate()) + " : " +this.count;
        }
    }
